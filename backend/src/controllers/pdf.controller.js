...
const getalluserpdfs=asyncHandler(async (req, res) => {
    ...
    if(!userpdfs||userpdfs.length===0){
        return res.status(404).json(new error_structurer(404,"pdfs not found",[]));
    }
...
const uploadpdf=asyncHandler(async(req,res)=>{
    if(!req.file){
        return res.status(400).json(new error_structurer(400,"No PDF file uploaded",null));
    }
    ...
    if(!savedpdf){
        return res.status(500).json(new error_structurer(500,"PDF not saved",null));
    }
